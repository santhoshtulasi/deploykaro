...
function PersonaCard({ p, selected, onSelect }: { p: typeof PERSONAS[0]; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      className={`relative p-6 rounded-3xl border-2 text-left transition-all duration-300 group ${selected ? colorMap[p.color] : "border-zinc-800 bg-zinc-900/40 hover:border-zinc-700"
        }`}
    >
      {selected && (
        <motion.div
      ...
      <div className="flex items-baseline gap-2 mb-1">
        ...
        <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border ${selected ? `${textMap[p.color]} border-current/30 bg-current/5` : "text-zinc-500 border-zinc-700"
          }`}>{p.lang}</span>
      </div>
    ...
    <main className="min-h-screen bg-zinc-950 text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-zinc-900/80 bg-zinc-950/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm text-zinc-400">
            ...
            <a
              href="/learning"
              className="flex items-center gap-1.5 text-emerald-400 hover:text-emerald-300 font-bold transition-colors border border-emerald-500/20 hover:border-emerald-500/40 px-3 py-1 rounded-lg"
            >
              <Play size={12} /> DeployAnything
            </a>
            <a
              href="/interview"
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center pt-16 px-6 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-5xl mx-auto">
          ...
          >
            {/* DeployAnything Learning Card */}
            <button
              id="hero-cta-learning"
              onClick={() => isAuthenticated ? router.push("/learning") : signIn("keycloak", { callbackUrl: "/learning" })}
              className="group flex flex-col items-start gap-4 p-8 rounded-[2rem] border border-zinc-800 bg-zinc-900/40 hover:bg-emerald-500/5 hover:border-emerald-500/40 transition-all text-left backdrop-blur-sm"
            >
              ...
              <div>
                <h3 className="font-black text-2xl text-white mb-2">DeployAnything Learn</h3>
                <p className="text-sm text-zinc-400 leading-relaxed h-10">Step-by-step guided deployment for any app. Choose your regional language.</p>
              </div>
              <div className="flex items-center gap-2 text-emerald-400 text-xs font-black uppercase tracking-widest mt-2 group-hover:translate-x-1 transition-transform">
                {isAuthenticated ? "Go to Learning" : "Start Learning"} <ArrowRight size={14} />
              </div>
            </button>
            ...
            <button
              ...
              onClick={() => isAuthenticated ? router.push("/interview") : signIn("keycloak", { callbackUrl: "/interview" })}
              className="group flex flex-col items-start gap-4 p-8 rounded-[2rem] border border-zinc-800 bg-zinc-900/40 hover:bg-purple-500/5 hover:border-purple-500/40 transition-all text-left backdrop-blur-sm"
            >
              ...
              <div>
                <h3 className="font-black text-2xl text-white mb-2">AI Mock Interviews</h3>
                <p className="text-sm text-zinc-400 leading-relaxed h-10">Test your skills with scenario-based DevOps/MLOps questions. (English Only).</p>
              </div>
              <div className="flex items-center gap-2 text-purple-400 text-xs font-black uppercase tracking-widest mt-2 group-hover:translate-x-1 transition-transform">
                {isAuthenticated ? "Go to Interviews" : "Start Interview"} <ArrowRight size={14} />
              </div>
            </button>
          </motion.div>
        ...
        >
          {[
            ...
            <motion.div
              ...
              className={`p-4 rounded-2xl text-xs leading-relaxed border max-w-[280px] ${msg.role === "user"
